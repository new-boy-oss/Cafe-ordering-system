...
void Interface::fruitjuice_show()
{
	...
	setbkmode(TRANSPARENT);// 去掉文字背景
	settextcolor(RGB(0, 0, 15));
	outtextxy(330, 170, "No10");
	outtextxy(330, 270, "No11");
	outtextxy(330, 370, "No12");
	outtextxy(380, 170, "神仙玉葡提");
	outtextxy(380, 270, "桃气果果");
	outtextxy(380, 370, "奇异果百乐冰");
	setbkmode(TRANSPARENT);// 去掉文字背景
	settextcolor(RGB(0, 0, 15));
	settextstyle(15, 0, "楷体");
	outtextxy(320, 220, "货存：30");
	...
	outtextxy(320, 420, "货存：30");
	settextstyle(24, 0, "楷体");
	outtextxy(800, 210, "价格：19元");
	outtextxy(800, 310, "价格：14元");
	outtextxy(800, 410, "价格：18元");
	while (1)
	{
		m = getmessage();//获取鼠标操作
		if (m.x >= 200 && m.x <= 980 && m.y >= 150 && m.y <= 250) //检测鼠标的位置 是否满足条件
